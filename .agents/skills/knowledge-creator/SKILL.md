...
7. 任何可能向数据库插入新条目的操作，必须先向用户展示拟写入卡片的预览，并在单独的用户回合中取得明确确认。用户最初提出“新增”不等于确认预览；未确认前不得运行 `create`，也不得运行可能创建新条目的 `upsert`。
...
4. 明确询问用户是否确认写入，然后停止当前回合并等待答复。只有“确认”“写入”“可以”等清晰肯定答复才算授权；若用户要求修改，更新预览后必须再次确认。
5. 获得确认后，将最终条目写入临时 JSON 文件，运行 `create`。
...
3. 若条目不存在，按“创建”流程展示完整预览并取得明确确认后，才能运行 `upsert`。
...
补充理解这段经历所需的背景、前因后果或长期状态；写 2–3 段。
