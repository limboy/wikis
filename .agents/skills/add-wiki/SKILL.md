---
name: add-wiki
description: 创建、查询、修改或删除知识点卡片，并将每次数据变更持久化到 wikis SQLite 数据库。用户要求从文本或对话提炼知识、调整已有知识点、维护标签/关联、导入条目或删除条目时使用；按概念、观点、叙事、实体分类生成结构化字段和 Markdown 正文，并执行对应的数据库 CRUD 操作与回读验证。
---
...
- `entity`：人物、作品、组织、地点等具体实体，回答“它是谁/是什么”，强调身份、背景和关键事实。
...
interface KnowledgeEntry {
  id: string
  title: string
  type: 'concept' | 'viewpoint' | 'narrative' | 'entity'
  oneLiner: string
  content?: string
  source?: {
...
准确描述该实体的身份、类别与核心事实（如人物的身份和经历、作品的类型和内容、组织的性质和使命、地点的位置和特征）；写 2–3 段。

## 背景与脉络

交代该实体所处的历史、时代或语境，以及其起源和发展过程；写 2–3 段。

## 意义与关联

说明该实体的影响、与其他人物/事件/概念的关联，以及为何值得记录；写 2–3 段。
```
